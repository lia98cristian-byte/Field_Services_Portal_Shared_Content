(function(){
  ...
  function sortedWorkspaces(items){
    const names=new Set(BASE_WORKSPACES);
    for(const name of workspaceCounts(items).keys()) names.add(name);
    return [...names].sort((a,b)=>{
      const ai=BASE_WORKSPACES.indexOf(a), bi=BASE_WORKSPACES.indexOf(b);
      if(ai!==-1 || bi!==-1){
  ...
  function updateWorkspaceSelect(items){
    ...
    for(const name of names){
      const o=document.createElement('option');o.value=name;o.textContent=name;workspaceSelect.appendChild(o);
    }
  ...
  function renderWorkspaceNav(items){
    ...
    for(const name of names){
      ...
      b.innerHTML=`<span>${esc(name)}</span><span class="count">${counts.get(name)||0}</span>`;workspaceNav.appendChild(b);
    }
